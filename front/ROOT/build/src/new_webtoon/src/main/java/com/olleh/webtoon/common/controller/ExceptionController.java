...
public class ExceptionController {
	...
	protected static Log logger = LogFactory.getLog(ExceptionController.class);
	 ...
	 */
	@RequestMapping(value = "/common/exception.kt")
	public ModelAndView login(HttpServletRequest request) {
		
		ModelAndView modelAndView = new ModelAndView();
		
		String errorcd = RequestUtil.getParameter(request, "errorcd", true);
		...
		try {
			...
			String viewName = "pc/exception/";
			
			if(isMobile) {
				viewName = "mobile/exception/";
			}
			
			if(errorcd.equals("400")) {
				viewName += "bad_request";
			} else if(errorcd.equals("404")) {
				viewName += "not_found";
			} else {
				viewName += "exception";
			}
