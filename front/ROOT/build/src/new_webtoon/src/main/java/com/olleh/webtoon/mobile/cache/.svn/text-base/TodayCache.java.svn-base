...
package com.olleh.webtoon.mobile.cache;
...
public class TodayCache {
	...
	protected static Log logger = LogFactory.getLog(TodayCache.class);
	private static TodayCache todayCache = null;           //TodayCache 인스턴스
	private static ModelAndView cachedModelAndView = null; //캐싱된 ModelAndView 객체
	...
	/**
     * TodayCache 객체의 instance를 리턴한다.
     *
     * @return TodayCache todayCache : TodayCache todayCache
     */
    public static TodayCache getInstance() {

    	//TodayCache 객체가 null일 경우만 생성하는 싱글톤 패턴.
    	if(todayCache == null) {
    		todayCache = new TodayCache();
    	}
