...
$(document).ready(function(){
	...
	$(".help_bar > ul > li > .help_button").bind(
		"click", function() {
			if($(".help_bar").hasClass("active") == false){
				$(".help_bar").addClass("active").animate({"right":0},{duration:400, easing:"easeOutQuad"});
				return false;
			}else{
				$(".help_bar").removeClass("active").animate({"right":-113},{duration:400, easing:"easeOutQuad"});
			}
	});
	...
	$(function(){
		$(".navigation > ul > li > a").click(function(event){
			...
			$('html, body').stop().animate({scrollTop:$(this.hash).offset().top - 31}, 500);
	...
	$("a.button_term1_all").bind({
		"click" : function() {
			...
			if($(this).siblings(".term_box").length > 0){
				if($(this).siblings(".term_box").hasClass("on")){
					$(this).siblings(".term_box").removeClass("on");
				   $(this).siblings(".term_box").stop().animate({height:txtMinHeight});
					return false;
				}else{
					$(this).siblings(".term_box").addClass("on");
					$(this).siblings(".term_box").stop().animate({height:txtMaxHeight});
					return false;
				};
			}else{
				return false;
